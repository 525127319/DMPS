...
export default class WeekTable extends Component {
    ...
    render() {
        let { weekpartmentdata } = this.props;
        let { shift } = this.props;

        if (weekpartmentdata && weekpartmentdata.children) {
            weekpartmentdata = DepartmentStatusUtil.matchByShiftId(weekpartmentdata, shift);
        }
        ...
        return (
            <div className='dev-table'>
                <ul className='data-list'>
                    <li>
                        <span>工作时长(h)</span>
                        <p>{DepartmentStatisticUtil.parseTime(weekpartmentdata.wt)}</p>
                    </li>
                    <li>
                        <span>开机时长(h)</span>
                        <p>{DepartmentStatisticUtil.parseTime(weekpartmentdata.rt)}</p>
                    </li>
                    <li>
                        <span>稼动率(%)</span>
                        <p>{weekpartmentdata.efficiency || 0}</p>
                    </li>
                    <li>
                        <span>报警总数</span>
                        <p>{weekpartmentdata.alarm_count || 0}</p>
                    </li>
                    <li>
                        <span>报警总时长(h)</span>
                        <p>{DepartmentStatisticUtil.parseTime(weekpartmentdata.alarm_duration)}</p>
                    </li>
                    <li>
                        <span>异常加工次数</span>
                        <p style={styles.red}>{weekpartmentdata.invalid_count || 0}</p>
                    </li>
                    <li>
                        <span>总加工次数</span>
                        <p style={styles.blue}>{weekpartmentdata.count || 0}</p>
